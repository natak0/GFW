...
export const FOURWINGS_INTERVALS_ORDER: FourwingsInterval[] = [
  'HOUR',
  'DAY',
  'MONTH',
  'YEAR',
...
export const LIMITS_BY_INTERVAL: Record<
  FourwingsInterval,
  | { unit: 'days' | 'months' | 'year'; value: number; buffer: number }
  | undefined
> = {
  HOUR: {
    unit: 'days',
...
export const getFourwingsInterval = (
  ...
  const startMillis =
    typeof start === 'string' ? DateTime.fromISO(start).toMillis() : start;
  const endMillis =
    typeof end === 'string' ? DateTime.fromISO(end).toMillis() : end;

  if (!startMillis && !endMillis) {
    return availableIntervals[0];
  }
  const duration = Duration.fromMillis(endMillis - startMillis);
  const validIntervals = Object.entries(LIMITS_BY_INTERVAL).flatMap(
    ([interval, limits]) => {
      if (!availableIntervals.includes(interval as FourwingsInterval))
        return [];
